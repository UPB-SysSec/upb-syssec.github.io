...
If the server supports HTTP/2, it can process the preface and respond with its own connection preface, containing a **SETTINGS** frame.
<!-- TODO: is this correct?-->
After receiving the servers' **SETTINGS** frame, the client sends its HTTP/2 request.
...
<!-- Todo: change to answers instead of questions? then we can also add the tool as contribution-->
In the context of censorship, HTTP/2 has not been analyzed extensively.
In this blogpost, we analyze HTTP/2's viability as a censorship circumvention technique by answering the following questions.
1. Is HTTP/2 censored in countries that employ HTTP/1.1 censorship?
2. How many servers support unencrypted HTTP/2 with either prior knowledge or the upgrade mechanism?
3. How many servers support unencrypted HTTP/1.1 in comparison with unencrypted HTTP/2?

Besides showing the viability of unencrypted HTTP/2 as a censorship circumvention technique, we provide [`Does-It-Support-Unencrypted-Http`](https://github.com/UPB-SysSec/Does-It-Support-Unencrypted-Http), a tool that analyzes a server for unencrypted HTTP support across multiple versions.

<!-- TODO: I would remove this sentence: In the following, we give an updated look on HTTP censorship in China and Iran.-->
...
HTTP/1.1 censorship in China and Iran can be triggered with a simple curl command (`curl http://nsfwyoutube.com`).
...
For instance, `curl --http2-prior-knowledge http://nsfwyoutube.com` accesses a website censored over HTTP/1.1 in both countries.
